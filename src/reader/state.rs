...
use crate::utils::{is_whitespace, name_len, to_u64};
    ...
    pub fn emit_bang<'b>(&mut self, bang_type: BangType, buf: &'b [u8]) -> Result<Event<'b>> {
        ...
        match bang_type {
                ...
                if self.config.check_comments {
                    ...
                    while let Some(p) = memchr::memchr(b'-', haystack) {
                        ...
                        if buf[3 + off] == b'-' {
                            ...
                            //     ^   :           - self.offset - len + 2
                            //         ^           - self.offset - len + 2 + p
                            self.last_error_offset = self.offset - to_u64(len) + 2 + to_u64(p);
            ...
            _ => {
                ...
                self.last_error_offset = self.offset - to_u64(len) - 2;
            ...
            Some(start) => {
                if self.config.check_end_names {
                    ...
                    if name != expected {
                        ...
                        // -2 for `<` and `>`
                        self.last_error_offset = self.offset - to_u64(buf.len()) - 2;
